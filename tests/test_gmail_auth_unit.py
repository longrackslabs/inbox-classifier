...
from inbox_classifier.gmail_auth import get_gmail_service, SCOPES
...
class TestGetGmailService:
    ...
    @patch('inbox_classifier.gmail_auth.Path')
    @patch('builtins.open', new_callable=mock_open)
    def test_refresh_failure_fallback(self, mock_file, mock_path_class, mock_creds_class,
                                     mock_request, mock_flow_class, mock_build):
        """Test that refresh failure falls back to re-authentication."""
        # Setup mocks
        mock_token_path = Mock()
        mock_token_path.exists.return_value = True
        mock_token_path.parent.mkdir = Mock()
        ...
        # Mock flow for re-authentication
        mock_flow = Mock()
        mock_new_creds = Mock()
        ...
        # Verify
        assert result == mock_service
        mock_old_creds.refresh.assert_called_once()
    ...
    @patch('inbox_classifier.gmail_auth.Path')
    @patch('builtins.open', new_callable=mock_open)
    def test_creates_token_directory(self, mock_file, mock_path_class, mock_flow_class, mock_build):
    ...
    @patch('inbox_classifier.gmail_auth.Path')
    @patch('builtins.open', new_callable=mock_open)
    def test_new_authentication_flow(self, mock_file, mock_path_class, mock_flow_class, mock_build):
